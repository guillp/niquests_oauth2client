...
def test_client_credentials_grant(
    ...
    client_credentials_grant_validator(niquests_mock.last_request, scope=scope)
...
def test_authorization_code_grant(
    ...
    assert niquests_mock.called_once
    authorization_code_grant_validator(niquests_mock.last_request, code=authorization_code)
...
def test_userinfo(
    ...
    userinfo = {"sub": sub}
    niquests_mock.post(userinfo_endpoint, json=userinfo)
    ...
    assert niquests_mock.last_request.headers["Accept"] == "application/json"
...
def test_userinfo_error(
    ...
    niquests_mock.post(userinfo_endpoint, json={"error": "access_denied"}, status_code=401)
...
def test_from_discovery_endpoint(
    ...
    niquests_mock.get(jwks_uri, json=as_public_jwks.to_dict())

    client = OAuth2Client.from_discovery_endpoint(discovery_url, issuer, auth=client_auth_method)

    assert niquests_mock.request_history[0].url == discovery_url
    assert niquests_mock.request_history[1].url == jwks_uri
...
def test_invalid_token_response(niquests_mock: NiquestsMocker, token_endpoint: str, client_id: str) -> None:
    """Token Endpoint error responses outside the standard raises an InvalidTokenResponse."""
    client = OAuth2Client(token_endpoint, auth=client_id)
    niquests_mock.post(token_endpoint, status_code=500, json={"confusing": "data"})
...
def test_invalid_token_response_200(niquests_mock: NiquestsMocker, token_endpoint: str, client_id: str) -> None:
    """Token Endpoint successful responses outside the standard raises an InvalidTokenResponse."""
    client = OAuth2Client(token_endpoint, auth=client_id)
    niquests_mock.post(token_endpoint, status_code=200, json={"confusing": "data"})
    with pytest.raises(InvalidTokenResponse):
        client.authorization_code("mycode")
...
def test_revoke_access_token(
    ...
    assert niquests_mock.called_once
    revocation_request_validator(niquests_mock.last_request, token=access_token, type_hint="access_token")
...
def test_revoke_refresh_token(
    ...
    assert niquests_mock.called_once
    revocation_request_validator(niquests_mock.last_request, token=refresh_token, type_hint="refresh_token")
    if client_auth_method_handler == PublicApp:
        public_app_auth_validator(niquests_mock.last_request, client_id=client_id)
    elif client_auth_method_handler == ClientSecretPost:
        client_secret_post_auth_validator(
...
def test_revoke_token(
    ...
    client = OAuth2Client(token_endpoint, revocation_endpoint=revocation_endpoint, auth=client_auth_method)
    niquests_mock.post(revocation_endpoint, status_code=200)

    assert client.revoke_token(refresh_token) is True

    assert niquests_mock.called_once
    revocation_request_validator(niquests_mock.last_request, refresh_token)
...
def test_revoke_token_error(
    ...
    client = OAuth2Client(token_endpoint, revocation_endpoint=revocation_endpoint, auth=client_auth_method)
    niquests_mock.post(revocation_endpoint, status_code=400, json={"error": "server_error"})
...
def test_revoke_token_error_non_standard(
    ...
    """
    client = OAuth2Client(token_endpoint, revocation_endpoint=revocation_endpoint, auth=client_auth_method)
    niquests_mock.post(revocation_endpoint, status_code=400, text="Error")

    assert client.revoke_token(refresh_token) is False
    assert niquests_mock.called_once

    niquests_mock.reset_mock()
    niquests_mock.post(revocation_endpoint, status_code=400, json={"foo": "bar"})
    assert client.revoke_token(refresh_token) is False
    assert niquests_mock.called_once
...
def test_server_jwks(
    ...
    niquests_mock.get(jwks_uri, json=server_public_jwks.to_dict())
...
def test_server_jwks_not_json(niquests_mock: NiquestsMocker, token_endpoint: str, jwks_uri: str) -> None:
    """If JWKS URI is not known, get_public_jwks() raises an exception."""
    niquests_mock.get(jwks_uri, text="Hello World!")
...
def test_server_jwks_invalid_doc(niquests_mock: NiquestsMocker, token_endpoint: str, jwks_uri: str) -> None:
    """If JWKS URI is an invalid document, get_public_jwks() raises an exception."""
    niquests_mock.get(jwks_uri, json={"foo": "bar"})
    client = OAuth2Client(token_endpoint=token_endpoint, jwks_uri=jwks_uri, auth=("foo", "bar"))
...
def test_introspection(
    ...
    niquests_mock.post(introspection_endpoint, json=introspection_data)
    data = oauth2client.introspect_token("access_token")
    assert data == introspection_data
    assert niquests_mock.called_once
    introspection_request_validator(niquests_mock.last_request, token="access_token")
...
def test_introspection_error(
    ...
    niquests_mock.post(introspection_endpoint, status_code=400, json={"error": "unauthorized_client"})
...
def test_introspection_jwt(
    ...
    niquests_mock.post(introspection_endpoint, text=introspection)
...
def test_introspection_unknown_error(
    ...
    niquests_mock.post(introspection_endpoint, status_code=400, text="Error")
    ...
    niquests_mock.reset_mock()
    niquests_mock.post(introspection_endpoint, status_code=400, json={"foo": "bar"})
    with pytest.raises(UnknownIntrospectionError):
        oauth2client.introspect_token("access_token")
...
def test_introspection_with_bearer_token_as_param(
    ...
    niquests_mock.post(introspection_endpoint, status_code=200, json={"active": False})
    bearer = BearerToken(access_token, refresh_token=refresh_token)
    assert oauth2client.introspect_token(bearer, "refresh_token")
    assert niquests_mock.called_once
    introspection_request_validator(niquests_mock.last_request, token=refresh_token, type_hint="refresh_token")

    niquests_mock.reset()
    assert oauth2client.introspect_token(bearer, token_type_hint="access_token")
    assert niquests_mock.called_once
    introspection_request_validator(niquests_mock.last_request, token=access_token, type_hint="access_token")
...
def test_ciba(
    ...
    niquests_mock.post(token_endpoint, json={"access_token": access_token, "expires_in": 60})
...
def test_pushed_authorization_request_error(
    ...
    niquests_mock.post(pushed_authorization_request_endpoint, json={"error": "server_error"}, status_code=500)

    with pytest.raises(ServerError):
        oauth2client.pushed_authorization_request(authorization_request)

    niquests_mock.post(pushed_authorization_request_endpoint, text="foobar", status_code=500)

    with pytest.raises(InvalidPushedAuthorizationResponse):
        oauth2client.pushed_authorization_request(authorization_request)


def test_jwt_bearer_grant(niquests_mock: NiquestsMocker, oauth2client: OAuth2Client, token_endpoint: str) -> None:
...
def test_custom_token_type(niquests_mock: NiquestsMocker, token_endpoint: str) -> None:
...
def test_client_custom_auth_method() -> None:
    class CustomAuthHandler(niquests.auth.AuthBase):
...
def test_proxy_authorization(niquests_mock: NiquestsMocker, target_api: str) -> None:
    ...
    niquests.post(target_api, auth=ProxyAuthorizationBearerToken(access_token))
    assert niquests_mock.last_request is not None
    assert niquests_mock.last_request.headers[auth_header] == f"Bearer {access_token}"


def test_custom_ports_in_endpoints(niquests_mock: NiquestsMocker) -> None:
    ...
    assert not niquests_mock.called_once
    ...
    niquests_mock.get(discovery_url, json={"issuer": issuer, "token_endpoint": token_endpoint})
