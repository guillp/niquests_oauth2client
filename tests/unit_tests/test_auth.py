...
def test_bearer_auth(
    ...
    response = niquests.post(target_api, auth=bearer_token)
    assert response.ok
    assert niquests_mock.last_request is not None
    assert niquests_mock.last_request.headers.get("Authorization") == f"Bearer {access_token}"
...
def test_expired_token(minutes_ago: datetime) -> None:
    ...
    with pytest.raises(ExpiredAccessToken):
        niquests.post("http://localhost/test", auth=token)
...
def test_access_token_auth(
    ...
    niquests.post(target_uri, auth=auth)

    assert len(niquests_mock.request_history) == 2
    refresh_request = niquests_mock.request_history[0]
...
def test_client_credentials_auth(
    ...
    niquests.post(target_api, auth=auth)

    assert len(niquests_mock.request_history) == 2
    cc_request = niquests_mock.request_history[0]
    api_request = niquests_mock.request_history[-1]
    ...
    niquests.post(target_api, auth=auth)
    assert len(niquests_mock.request_history) == 1
...
def test_authorization_code_auth(
    ...
    assert len(niquests_mock.request_history) == 2
    code_request = niquests_mock.request_history[0]
    api_request = niquests_mock.request_history[-1]
...
def test_ropc_auth(
    ...
    assert niquests.post(target_api, auth=auth).ok
...
def test_device_code_auth(
    ...
    assert niquests.post(target_api, auth=auth)
    assert len(niquests_mock.request_history) == 2
    device_code_request = niquests_mock.request_history[0]
    api_request = niquests_mock.request_history[1]
